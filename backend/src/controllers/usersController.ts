...
export const getAuthenticatedUser: RequestHandler = async(req,res,next) => {
    const authticatedUserId = req.session.userId;

    try {
        

        const user = await UserModel.findById(authticatedUserId).select("+email").exec();
...
export const login: RequestHandler<unknown,unknown,LoginBody,unknown> = async (req,res,next) => {
    ...
    try {
        ...
        if(!user){
            throw createHttpError(401, "username does not exists!");
        }
